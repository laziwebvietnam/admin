...
<form id="form-submit" class="form-horizontal" action="<?=$this->_table?>/action"
    ...
    <div class="row">
        <div class="col-md-8">
            ...
            <div class="portlet light bordered">
                <?
                    create_tab('Giá và hình ảnh');
                ?>
                <div class="portlet-body form">
                    <?
                        $input_price = array(
                            array('name'=>'price','title'=>lang('price'),'val'=>$data['detail']['price']),
                            array('name'=>'price_promotion','title'=>lang('price_promotion'),'val'=>$data['detail']['price_promotion'])
                        );
                        create_input_multy('Giá',$input_price,true);
                        $input_other = array(
                            array('name'=>'id_product','title'=>lang('id_product'),'val'=>$data['detail']['id_product']),
                            array('name'=>'quantity','title'=>lang('quantity'),'val'=>$data['detail']['quantity'])
                        );
                        create_input('weight','Khối lượng (kg)',$data['detail']['weight']);
                        create_input_multy('Khác',$input_other);
                        create_image('image',lang('image'),$data['detail']['image'],true);
                        create_image('images',lang('images'),$data['detail']['images'],false,true);
                    ?>
                </div>
            </div>
            <?
                $this->load->view($this->_base_view_path.'view/product/load/property');
            ...
            <div class="portlet-body form">
                <div class="form-body">
                    <?
                        ...
                        create_select_('id_product_color',null,$data['detail']['id_product_color'],null,$data['pro_color'],null,null,0);
                    ?>
                    <div class="form-group">
                    	<div class="col-md-12">
                    		<a href="category/product" target="_blank"><i class="fa fa-plus"></i> Thêm loại sản phẩm</a>
                    ...
                    <?
                        ...
                        create_checkbox('is_new',lang('is_new'),1,$data['detail']!=null?$data['detail']['is_new']:0,false,null,4);
                        create_checkbox('is_promotion',lang('is_promotion'),1,$data['detail']!=null?$data['detail']['is_promotion']:0,false,null,4);
                        create_checkbox('is_stock',lang('is_stock'),1,$data['detail']!=null?$data['detail']['is_stock']:1,false,null,4);
        ...
        <?
            ...
            $this->load->view($this->_base_view_path.'view/product/load/buy_together',$data);
